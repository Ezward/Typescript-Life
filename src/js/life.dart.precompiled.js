...
["Arrays", "../dart/Arrays.dart", , B, {
  "^": "",
  newArray2d: function(rows, columns) {
    var theArray, i;
    if (rows <= 0 || columns <= 0)
      throw H.wrapException("newArray2d() pass non-positive array size.");
    theArray = Array(rows);
    for (i = 0; i < rows; ++i)
      theArray[i] = Array(columns);
    return theArray;
  }
}],
["RenderMagnifiedLife", "../dart/RenderMagnifiedLife.dart", , K, {
  "^": "",
  CanvasRenderer: {
    "^": "Object;_canvas,_context,_magnification,_rendering,_fillColor",
    CanvasRenderer$1: function(theCanvas) {
      this._canvas = theCanvas;
      this._context = J.getContext$1$x(theCanvas, "2d");
      this._magnification = 1;
    }
  }
}],
["", "../dart/WorldOfLife.dart", , Z, {
  "^": "",
  _Individual: {
    "^": "Object;_row,_column,_id,_neighbors"
  },
  World: {
    "^": "Object;_rows,_columns,_world",
    _getIndividualXY$2: function(x, y) {
      var t1;
      if (y < 0 || y >= this._rows)
        throw H.wrapException("Population.getIndividualXY() row is out of range.");
      if (x < 0 || x >= this._columns)
        throw H.wrapException("Population.getIndividualXY() column is out of range.");
      t1 = this._world;
      if (y < 0 || y >= t1.length)
        return H.ioore(t1, y);
      t1 = t1[y];
    ...
    World$2: function(rows, columns) {
      var theId, theRowIndex, t1, theRow, theColumnIndex, t2, t3, theAboveRow, theBelowRow, theIndividual, theLeftIndex, theColumnIndex0, theRightIndex, t4;
      if (rows <= 0 || columns <= 0)
        throw H.wrapException("World.constructor() was passed non-positive dimensions.");
      this._rows = rows;
      this._columns = columns;
      this._world = B.newArray2d(rows, columns);
      for (theId = 1, theRowIndex = 0; theRowIndex < rows; ++theRowIndex) {
        t1 = this._world;
        if (theRowIndex >= t1.length)
          return H.ioore(t1, theRowIndex);
        theRow = t1[theRowIndex];
        for (theColumnIndex = 0; theColumnIndex < columns; ++theColumnIndex) {
          t1 = C.JSInt_methods.toString$0(theId);
          t2 = new Z._Individual(null, null, null, []);
          t2._id = t1;
          t2._row = theRowIndex;
          t2._column = theColumnIndex;
          if (theColumnIndex >= theRow.length)
            return H.ioore(theRow, theColumnIndex);
          theRow[theColumnIndex] = t2;
          ++theId;
        }
      }
      for (theRowIndex = 0; theRowIndex < rows;) {
        t1 = this._world;
        t2 = C.JSInt_methods.$mod(rows + theRowIndex - 1, rows);
        t3 = t1.length;
        if (t2 >= t3)
        ...
        theRow = t1[theRowIndex];
        ++theRowIndex;
        t2 = C.JSInt_methods.$mod(theRowIndex, rows);
        if (t2 >= t3)
          return H.ioore(t1, t2);
        theBelowRow = t1[t2];
        for (theColumnIndex = 0; theColumnIndex < columns; theColumnIndex = theColumnIndex0) {
          t1 = theRow.length;
          if (theColumnIndex >= t1)
            return H.ioore(theRow, theColumnIndex);
          theIndividual = theRow[theColumnIndex];
          theLeftIndex = C.JSInt_methods.$mod(columns + theColumnIndex - 1, columns);
          theColumnIndex0 = theColumnIndex + 1;
          theRightIndex = C.JSInt_methods.$mod(theColumnIndex0, columns);
          t2 = theAboveRow.length;
          if (theLeftIndex >= t2)
            return H.ioore(theAboveRow, theLeftIndex);
          t3 = theAboveRow[theLeftIndex];
          t4 = theIndividual._neighbors;
          t4.push(t3);
          if (theColumnIndex >= t2)
            return H.ioore(theAboveRow, theColumnIndex);
          t4.push(theAboveRow[theColumnIndex]);
          if (theRightIndex >= t2)
            return H.ioore(theAboveRow, theRightIndex);
          t4.push(theAboveRow[theRightIndex]);
          if (theLeftIndex >= t1)
            return H.ioore(theRow, theLeftIndex);
          t4.push(theRow[theLeftIndex]);
          if (theRightIndex >= t1)
            return H.ioore(theRow, theRightIndex);
          t4.push(theRow[theRightIndex]);
          t1 = theBelowRow.length;
          if (theLeftIndex >= t1)
            return H.ioore(theBelowRow, theLeftIndex);
          t4.push(theBelowRow[theLeftIndex]);
          if (theColumnIndex >= t1)
            return H.ioore(theBelowRow, theColumnIndex);
          t4.push(theBelowRow[theColumnIndex]);
          if (theRightIndex >= t1)
            return H.ioore(theBelowRow, theRightIndex);
          t4.push(theBelowRow[theRightIndex]);
        }
      }
    },
    static: {World$: function(rows, columns) {
        var t1 = new Z.World(null, null, null);
        t1.World$2(rows, columns);
        return t1;
      }}
  },
  Population: {
    "^": "Object;_world,_wasAlive,_isAlive,_touched,_toRender,_neighborCount",
    makeAliveXY$2: function(x, y) {
      var theIndividual = this._world._getIndividualXY$2(x, y);
      if (null != theIndividual)
        this._makeAlive$1(theIndividual);
    },
    _makeAlive$1: function(theIndividual) {
      var theIndividualId = theIndividual._id;
      this._isAlive.$indexSet(0, theIndividualId, theIndividual);
      this._touched.$indexSet(0, theIndividualId, theIndividual);
      this._toRender.$indexSet(0, theIndividualId, theIndividual);
    },
    nextGeneration$0: function() {
      var t1, t2, theId, theIndividual, j, theNeighbor, theNeighborId, t3, t4;
      t1 = this._isAlive;
      this._wasAlive = t1;
      this._toRender = this._touched;
      this._isAlive = P.LinkedHashMap_LinkedHashMap$_empty(null, null);
      this._touched = P.LinkedHashMap_LinkedHashMap$_empty(null, null);
      this._neighborCount = P.LinkedHashMap_LinkedHashMap$_empty(null, null);
      for (t1 = t1.get$keys()._map, t2 = new P.LinkedHashMapKeyIterator(t1, t1._modifications, null, null), t2._cell = t1._first; t2.moveNext$0();) {
        theId = t2._current;
        theIndividual = this._wasAlive.$index(0, theId);
        this._touched.$indexSet(0, theId, theIndividual);
        for (j = 0; j < 8; ++j) {
          theIndividual.toString;
          if (j >= theIndividual._neighbors.length)
            H.throwExpression("_Individual.getNeighbor() index is out of range.");
          t1 = theIndividual._neighbors;
          if (j >= t1.length)
            return H.ioore(t1, j);
          theNeighbor = t1[j];
          theNeighborId = theNeighbor._id;
          t1 = this._neighborCount;
          if (t1.containsKey$1(theNeighborId)) {
            t3 = this._neighborCount.$index(0, theNeighborId);
            if (typeof t3 !== "number")
              return t3.$add();
            ++t3;
          } else
            t3 = 1;
          t1.$indexSet(0, theNeighborId, t3);
          switch (this._neighborCount.$index(0, theNeighborId)) {
            case 2:
              if (null != this._wasAlive.$index(0, theNeighbor._id))
                this._isAlive.$indexSet(0, theNeighborId, theNeighbor);
              break;
            case 3:
              this._isAlive.$indexSet(0, theNeighborId, theNeighbor);
              break;
            case 4:
              this._isAlive.remove$1(0, theNeighborId);
              break;
          }
        }
      }
      for (t1 = this._isAlive.get$keys()._map, t2 = new P.LinkedHashMapKeyIterator(t1, t1._modifications, null, null), t2._cell = t1._first; t2.moveNext$0();) {
        theId = t2._current;
        t1 = this._toRender;
        t3 = this._touched;
        t4 = this._isAlive.$index(0, theId);
        t3.$indexSet(0, theId, t4);
        t1.$indexSet(0, theId, t4);
      }
    },
    render$1: function(theRenderer) {
      var t1, t2, theId, theIndividual, t3, t4, t5, t6, theFillColor, theFillColor0;
      for (t1 = this._toRender.get$keys()._map, t2 = new P.LinkedHashMapKeyIterator(t1, t1._modifications, null, null), t2._cell = t1._first; t2.moveNext$0();) {
        theId = t2._current;
        theIndividual = this._toRender.$index(0, theId);
        t1 = theIndividual._column;
        t3 = theIndividual._row;
        t4 = null != this._isAlive.$index(0, theIndividual._id);
        t5 = null == this._wasAlive.$index(0, theIndividual._id);
        t6 = !t5;
        if (!t5 || t4) {
          theFillColor = theRenderer._fillColor;
          if (t4)
            theFillColor0 = t6 ? "blue" : "yellow";
          else
            theFillColor0 = t6 ? "red" : theFillColor;
          if (theFillColor0 == null ? theFillColor != null : theFillColor0 !== theFillColor) {
            t4 = theRenderer._context;
            theRenderer._fillColor = theFillColor0;
            J.set$fillStyle$x(t4, theFillColor0);
          }
          t4 = theRenderer._context;
          t5 = theRenderer._magnification;
          if (typeof t5 !== "number")
            return H.iae(t5);
          J.fillRect$4$x(t4, t1 * t5, t3 * t5, t5, t5);
        } else {
          t4 = theRenderer._context;
          t5 = theRenderer._magnification;
          if (typeof t5 !== "number")
            return H.iae(t5);
          J.clearRect$4$x(t4, t1 * t5, t3 * t5, t5, t5);
        }
      }
    },
    Population$1: function(theWorld) {
      this._world = theWorld;
      this._wasAlive = P.LinkedHashMap_LinkedHashMap$_empty(null, null);
      this._isAlive = P.LinkedHashMap_LinkedHashMap$_empty(null, null);
      this._touched = P.LinkedHashMap_LinkedHashMap$_empty(null, null);
      this._toRender = P.LinkedHashMap_LinkedHashMap$_empty(null, null);
      this._neighborCount = P.LinkedHashMap_LinkedHashMap$_empty(null, null);
    }
  }
}],
...
["dart.async", "dart:async", , P, {
  ...
  _Future: {
    ...
    _addListener$1: function(listener) {
      ...
      if (this._state >= 4) {
        t1 = this._zone;
        f = new P._Future__addListener_closure(this, listener);
        t1.toString;
        P._scheduleAsyncCallback(C.C__RootZone !== t1 ? t1.bindCallback$1(f) : f);
      } else {
    ...
    $is_Future: true,
    $isFuture: true,
    static: {"^": "_Future__INCOMPLETE,_Future__PENDING_COMPLETE,_Future__CHAINED,_Future__VALUE,_Future__ERROR", _Future$: function() {
        return new P._Future(0, $.Zone__current, null, null, null, null, null, null);
  ...
  _Future__chainForeignFuture_closure0: {
    ...
    call$2: function(error, stackTrace) {
      ...
      t1 = this.target_1;
      listeners = t1._removeListeners$0();
      t1._setError$2(error, stackTrace);
      P._Future__propagateToListeners(t1, listeners);
    },
    call$1: function(error) {
  ...
  _Future__propagateToListeners_handleValueCallback: {
    "^": "Closure:12;box_1,listener_3,sourceValue_4,zone_5",
    call$0: function() {
...
["dart.dom.html", "dart:html", , W, {
  ...
  Window: {
    ...
    get$animationFrame: function(receiver) {
      var completer = H.setRuntimeTypeInfo(new P._SyncCompleter(P._Future$()), [P.num]);
  ...
  Window_animationFrame_closure: {
    ...
    call$1: function(time) {
      ...
      t2 = J.getInterceptor(time);
      if (!!t2.$isFuture)
        if (!!t2.$is_Future)
          P._Future__chainCoreFuture(time, t1);
        else
          P._Future__chainForeignFuture(time, t1);
      else {
        listeners = t1._removeListeners$0();
        t1._setValue$1(time);
        P._Future__propagateToListeners(t1, listeners);
...
["", "../dart/main.dart", , F, {
  "^": "",
  main: [function() {
    var theStage, t1, t2, theRows, theColumns, theInitialPopulation, thePopulation, i, theIndividual, theRenderer;
    theStage = document.getElementById("stage-canvas");
    t1 = J.getInterceptor$x(theStage);
    t1.set$width(theStage, theStage.clientWidth);
    t1.set$height(theStage, theStage.clientHeight);
    t2 = t1.get$height(theStage);
    if (typeof t2 !== "number")
      return t2.$div();
    theRows = C.JSNumber_methods.toInt$0(Math.floor(t2 / 4));
    t1 = t1.get$width(theStage);
    if (typeof t1 !== "number")
      return t1.$div();
    theColumns = C.JSNumber_methods.toInt$0(Math.floor(t1 / 4));
    theInitialPopulation = C.JSNumber_methods.toInt$0(C.JSInt_methods.roundToDouble$0(theRows * theColumns / 12));
    thePopulation = new Z.Population(null, null, null, null, null, null);
    thePopulation.Population$1(Z.World$(theRows, theColumns));
    thePopulation.makeAliveXY$2(1, 0);
    thePopulation.makeAliveXY$2(2, 1);
    thePopulation.makeAliveXY$2(0, 2);
    thePopulation.makeAliveXY$2(1, 2);
    thePopulation.makeAliveXY$2(2, 2);
    for (i = 0; i < theInitialPopulation; ++i) {
      t1 = C.C__JSRandom.nextInt$1(theColumns);
      t2 = C.C__JSRandom.nextInt$1(theRows);
      theIndividual = thePopulation._world._getIndividualXY$2(t1, t2);
      if (null != theIndividual)
        thePopulation._makeAlive$1(theIndividual);
    }
    theRenderer = new K.CanvasRenderer(null, null, null, false, null);
    theRenderer.CanvasRenderer$1(theStage);
    theRenderer._magnification = 4;
    new F.main_animationLoop(new F.main_animationFrame(thePopulation, theRenderer)).call$1(0);
  }, "call$0", "main$closure", 0, 0, 1],
  main_animationFrame: {
    "^": "Closure:15;thePopulation_0,theRenderer_1",
    call$1: function(theTimeDelta) {
      var t1, t2, t3;
      t1 = this.theRenderer_1;
      t2 = this.thePopulation_0;
      if (t1._rendering) {
  ...
  main_animationLoop: {
    "^": "Closure:15;animationFrame_2",
    call$1: function(theTimeDelta) {
      this.animationFrame_2.call$1(theTimeDelta);
...
function dart_precompiled($collectedClasses) {
  ...
  function CanvasRenderer(_canvas, _context, _magnification, _rendering, _fillColor) {
    this._canvas = _canvas;
    this._context = _context;
    this._magnification = _magnification;
  ...
  function _Individual(_row, _column, _id, _neighbors) {
    this._row = _row;
    this._column = _column;
    this._id = _id;
    this._neighbors = _neighbors;
  }
  _Individual.builtin$cls = "_Individual";
  if (!"name" in _Individual)
    _Individual.name = "_Individual";
  $desc = $collectedClasses._Individual;
  if ($desc instanceof Array)
    $desc = $desc[1];
  _Individual.prototype = $desc;
  function World(_rows, _columns, _world) {
    this._rows = _rows;
  ...
  function Population(_world, _wasAlive, _isAlive, _touched, _toRender, _neighborCount) {
    this._world = _world;
    this._wasAlive = _wasAlive;
    this._isAlive = _isAlive;
    this._touched = _touched;
    this._toRender = _toRender;
    this._neighborCount = _neighborCount;
  }
  Population.builtin$cls = "Population";
  if (!"name" in Population)
    Population.name = "Population";
  $desc = $collectedClasses.Population;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Population.prototype = $desc;
